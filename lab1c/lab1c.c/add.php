...
<option value="5">Add (Director or Actor)/Movie relation</option>
</select>
...
<?php
	...
	if ($_GET["did"]){
		...
		// FIXME try to find a way for the user to search again
?>
...
$genres = array("Action", "Adult", "Adventure", "Animation", "Comedy", "Crime",
	...
	echo "</p></div>";
?>
<p><input type="submit" value="Add Movie" <?php if(!$_GET["did"]) echo "disabled=\"disabled\""; ?>/></p>
</form>
</p>
...
if ($type == "5" && $_POST["select"] == "adddm"){
	...
	if (mysql_query($relation, $db)){
		...
		echo "<br/>View your new relation <a href='./movies.php?id=$mid'>here</a></p>";
	} else { echo "<b>Coudn't add director to movie!</b>"; }
...
if($_POST["smovie"] && ($_POST["but"] == "mov")){
	...
	if($result && mysql_num_rows($result) > 0){
		...
		echo "</p></div>";
	} else { echo "<br/>No movie found with search \"$raw\""; }
...
if($_POST["sactor"] && ($_POST["but"] == "act")){
	...
	if($result && mysql_num_rows($result) > 0){
		...
		echo "</p></div>";
	} else { echo "<br/>No actor found with search \"$raw\""; }
...
if($_POST["sdirector"] && ($_POST["but"] == "dir")){
	...
	if($result && mysql_num_rows($result) > 0){
		...
		echo "</p></div>";
	} else { echo "<br/>No director found with search \"$raw\""; }
...
if(($type == "1" || $type == "2") && $_POST["first"] && 
	...
	if (str_replace(" ", "", $firsttemp) == ""){ // no first name
		...
		if ($type == "1")
			if ($dodflag == 0)
				$addpersonq = "INSERT INTO Actor VALUES ($pid, '$last', '$first', '$sex', '$dob', NULL)";
			else	 
				$addpersonq = "INSERT INTO Actor VALUES ($pid, '$last', '$first', '$sex', '$dob', '$dod')";
		elseif ($type == "2") 
			// person is a director
			if($dodflag == 0)	
				$addpersonq = "INSERT INTO Director VALUES ($pid, '$last', '$first', '$dob', NULL)";
			else
