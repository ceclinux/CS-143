...
<option value="1" <?php if($_GET["type"] == 1) echo "selected=\"selected\""; ?>>Add Actor or Director</option>
...
<?php 
	if($_GET["type"] == 1) 
		echo "<h3>Add a new actor or director: </h3>"; ?>
...
&& $_POST["year"] && $_POST["MPAA"] && is_numeric($_POST["year"])){
	...
	for($x=0;$x<19;$x++){
		...
		// only add director if director is specified
		if ($did > 1)
...
} elseif ($type == "4" && $_GET["did"] && ($_POST["clicked"]=="23") && (!$_POST["title"] || !$_POST["company"]
|| !$_POST["year"] || !$_POST["MPAA"] || !is_numeric($_POST["year"]))) {
	echo "<b>";
	if (!$_POST["title"])
	...
	if($result && mysql_num_rows($result) > 0){
		...
		while($r = mysql_fetch_row($result)){
			...
			if ($_GET["aid"]){
				...
				$did = $_GET["did"];
				echo "<a href = './add.php?type=5&did=$did&mid=$r[1]'>";
			} else 
...
if($type == "1" && $_POST["first"] && 
	...
	$aflag = isset($_POST["actcheck"]); 	// actor flag to see if actor is selected
	$dflag = isset($_POST["dircheck"]); 	// director flag to see if director is selected
	...
	$addactor = ""; 						// blank query for now (determine later)
	$adddirector = "";
	...
	if (str_replace(" ", "", $firsttemp) == ""){ // no first name
		...
		// add information to actor table
		if ($dodflag == 0)
		...
		// add information to director table
		if($dodflag == 0)	
		...
		if($dflag){
			...
			echo "View your profile <a href='./directors.php?id=$pid'>here</a><br/> ";
			$success = 1;
...
elseif ($type=="1" && $_POST["clicked"] && (!$_POST["first"] || !$_POST["last"] || 
