...
class DoubleLinkedList {
	...
	/*
	 * Insert the PageId pid to the linked list.
	...
	/*
	 * Locate the PageId pid in the linked list. Didn't implement this yet because I don't see a use for it.
	 * @param pid[IN] the PageId to insert
	 * @param location[OUT] the Node where the PageId was found
	 * @return error code. 0 if no error.
	 */
	int locate(PageId pid, Node* location);
	
	/*
	 * Locate the tail and return the PageId contained there
	 * Will also remove the given Node and move the tail back.
	 * Used to traverse the list in reverse to get the parent nodes.
	 * @param pid[OUT] the PageId at that Node
	 * @return error code. 0 if no error.
	 */
	int getValueAtTail(PageId& pid);
	
	/*
	 * Will remove all entries from list and cut it down to size 0 (head == tail).
	 * @return error code. 0 if no error.
	 */
	int destructList();
...
class BTreeIndex {
 ...
 private:
  ...
  PageId   nextPid;		/// next PageId to get
  DoubleLinkedList list; /// List of PageIds
  char mode;
