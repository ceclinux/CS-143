...
class BTLeafNode {
  public:
    ...
    static const int LEAF_MAX_ENTRIES = 
        (PageFile::PAGE_SIZE - sizeof(PageId)) / LEAF_ENTRY_SIZE;
    static const int LEAF_FIRST_ENTRY = 0;
   ...
   /**
    * Initializes the leaf by setting all entries to empty, as well as the
    * pointer to the next leaf.
    */
    void initialize();

    PageId getNextNodePtrHelp(char* buffer);
    void setNextNodePtrHelp(char* buffer, PageId pid);
...
/**
 * Important Assumption: whenever a PageFile is passed to BTNonLeafNode, 
 * it is assumed that it has already been opened, 
 * and that we can call on 
 */


typedef struct {
  int key; 
  PageId right_pid;  // slot number. the first slot is 0

} NonLeafData;


// minus size of Page Id to keep into account that the first 4 bytes are to store the left most pid
const int MAX_NONLEAFDATA = ((PageFile::PAGE_SIZE - sizeof(PageId))/sizeof(NonLeafData));
...
class BTNonLeafNode {
  public:
    ...
    */
    ...
    PageId getStartPid() const;
    void setStartPid(PageId pid);
    
  private:
    
    // Private method used to move the entries around.
    void shift(const char* buffer, unsigned int size);
    
   /**
    * The main memory buffer for loading the content of the disk page 
    * that contains the node.
    *
    * This buffer is actually used to store a bunch of (key, 
    *
    */

    // NonLeafData buffer[NUM_OF_NONLEAFDATA];

    NonLeafData* getStartOfBuffer();
