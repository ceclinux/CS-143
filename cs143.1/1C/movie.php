...
if(isset($_POST['title'])) {
	...
	$increase_count_query = "UPDATE MaxMovieID SET id = " . $id;
...
//		echo $row['title'];
		array_push($arr, array("movie" => $row));
	}

	$query = sprintf("SELECT * FROM MovieGenre WHERE mid = %s", mysqli_real_escape_string($db, $id));
...
//		echo $row['title'];
	...
	array_push($arr, array("genre" => $suba));

	$suba = array();

	$query = sprintf("SELECT * FROM MovieDirector, Director WHERE mid = %s AND did = id", mysqli_real_escape_string($db, $id));
...
//		echo $row['title'];
	...
	array_push($arr, array("director" => $suba));
	$suba = array();

	$query = sprintf("SELECT * FROM MovieActor, Actor WHERE mid = %s AND aid = id", mysqli_real_escape_string($db, $id));
...
//		echo $row['title'];
	...
	array_push($arr, array("actor" => $suba));
